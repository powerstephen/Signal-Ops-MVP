...
export default function DataCentre() {
  ...
  return (
    <div>
      {importing && <ImportModal source={importing} onClose={()=>setImporting(null)} onComplete={()=>setImporting(null)} />}

      <div className="flex items-start justify-between mb-5">
        <div>
          <h2 className="text-lg font-semibold text-white mb-1">Data Centre</h2>
          <p className="text-slate-400 text-sm">Connect your data sources. {connCount>0 ? <span className="text-teal-400 font-medium">{connCount} source{connCount!==1?'s':''} active.</span> : <span className="text-slate-500">No sources connected yet.</span>}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative max-w-xs">
          ...
          <input type="text" placeholder="Search integrations..." value={search} onChange={e=>setSearch(e.target.value)} className="w-full bg-navy-800 border border-slate-700 rounded-lg pl-9 pr-4 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-teal-500" />
        </div>
        <div className="flex gap-2 flex-wrap">
          {categories.map(c => <button key={c} onClick={()=>setActiveCat(c)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${activeCat===c?'bg-teal-500 text-white':'bg-slate-800 text-slate-400 hover:text-white border border-slate-700'}`}>{c}</button>)}
        </div>
      </div>
